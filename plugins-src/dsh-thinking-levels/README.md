...
- **行内注入(主形态)**:官方「模型」页展开某模型编辑器后,该行内直接出现「思考等级」编辑卡——七档开关 + 每档线上拼写输入 + 保存按钮,就地在原本的模型行里编辑,不新开任何悬浮框。
- **浮动回退(兜底)**:官方 DOM 结构变化导致行内锚点全失时,自动挂起浮动编辑面板(provider 下拉 + 全模型列表)保住入口;行内注入恢复即自动收起。
